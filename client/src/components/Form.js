...
const Form = ({ localData, handleChange, handleGenerate, handleClear, errors }) => {
  return (
    <form className="pitch-form" onSubmit={(e) => e.preventDefault()}>
      ...
      <textarea name="problem" value={localData.problem} onChange={handleChange} className={errors.problem ? 'error-input' : ''} />
      ...
      <label>Запрос</label>
      <input name="goal" value={localData.goal} onChange={handleChange} className={errors.goal ? 'error-input' : ''} />
